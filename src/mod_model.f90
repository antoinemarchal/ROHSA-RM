...
contains
  ...
  pure function f_rmsf(x, a, m, cc)
    ...
    real(xp), intent(in), dimension(3*params%n_rmsf) :: cc
    real(xp) :: f_rmsf
    ...
    do i=1, params%n_rmsf
       f_rmsf = f_rmsf + a * (cc(1+(3*(i-1))) * exp(-(x - (m + cc(2+(3*(i-1)))) )**2._xp / (2._xp * cc(3+(3*(i-1)))**2)))
    end do
  ...
  pure function df_rmsf_da(x, m, cc)
    ...
    do i=1, params%n_rmsf
       df_rmsf_da = df_rmsf_da + (cc(1+(3*(i-1))) * exp(-((x - (m + cc(2+(3*(i-1)))) )**2._xp) / (2._xp * cc(3+(3*(i-1)))**2)))
    end do
  ...
  pure function df_rmsf_dm(x, a, m, cc)
    ...
    do i=1, params%n_rmsf
       df_rmsf_dm = df_rmsf_dm + a * (x - (m + cc(2+(3*(i-1)))) ) / cc(3+(3*(i-1)))**2 * (cc(1+(3*(i-1))) &
            * exp(-(x - (m + cc(2+(3*(i-1)))) )**2._xp / (2._xp * cc(3+(3*(i-1)))**2)))
    end do
